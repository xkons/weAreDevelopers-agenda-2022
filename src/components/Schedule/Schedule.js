...
import scheduleItems from '../../Resources/coderCruiseSchedule.json';
...
class Schedule extends Component {
  ...
  componentDidMount = () => {
    localForage.getItem('coderCruiseStarredTalks', (err, offlineStars) => {
  ...
  setLocation = (location) => this.setState({ "location": location, favoritesViewIsActive: false });

  getScheduleItems = (day, location) => scheduleItems.filter(talk => talk.day === day && talk.location === location);

  getScheduleFilteredByTalks = (talkIds) => scheduleItems.filter(talk => talkIds.includes(talk.id));

  addStar = (talkId) => this.setState(prevState => {
      const newStars = [...prevState.starredTalks, talkId];
      localForage.setItem('coderCruiseStarredTalks', newStars);
      return {starredTalks: newStars}
    }
  );

  removeStar = (talkId) => this.setState(prevState => {
      const newStars = prevState.starredTalks.filter(starredTalkId => starredTalkId !== talkId);
      localForage.setItem('coderCruiseStarredTalks', newStars);
      return {starredTalks: newStars}
    }
  );
  ...
  starTalkHandler = (talkId) => {
    this.state.starredTalks.includes(talkId) ? this.removeStar(talkId) : this.addStar(talkId);
  };
          ...
          <Segment>
            {this.state.favoritesViewIsActive ? 
              ...
              <ScheduleList
                starredTalks={this.state.starredTalks}
                starTalkHandler={this.starTalkHandler}
                items={this.getScheduleItems(this.state.day, this.state.location)} />
            }
          </Segment>
        </main>
      </div>
